/**
 * MonsoonGuardian ad capture (?demo=monsoonguardian&autostart=0)
 * Report flow + Me Too on neighbour pin — cursor overlay, flow marks for voice sync.
 */
(function () {
  'use strict';

  const params = new URLSearchParams(location.search);
  if (params.get('demo') !== 'monsoonguardian') return;
  ...
  async function openNeighborMeToo() {
    ...
    if (btn) {
      await flowClick(btn, { hold: 380, moveMs: 320 });
      await sleep(1800);
    } else {
